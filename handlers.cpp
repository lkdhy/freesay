...
std::size_t get_user_num(
	bserv::db_transaction& tx
) {
	bserv::db_result db_res =
        tx.exec("select count(*) from auth_user where is_active;");
	lginfo << db_res.query();
	std::size_t total_users = (*db_res.begin())[0].as<std::size_t>();
    return total_users;
}

// added: list all the users, 10 users a page
boost::json::object list_users(
	...
	bserv::db_result db_res;
	std::size_t total_users = get_user_num(tx);
...
boost::json::object user_register(
	...
	int total_users_0 = get_user_num(tx);
